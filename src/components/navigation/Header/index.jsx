...
export default function Header() {
  ...
  return (
    <header className="w-full bg-white sticky top-0 z-50 relative font-['Almarena']">
      ...
      {/* Collection Mega Menu */}
      {activeDropdown === 'collection' && (
        <div className="hidden md:block absolute top-full left-1/2 -translate-x-1/2 w-[95%] max-w-[1340px] bg-white border border-black rounded-b-[32px] rounded-t-[16px] shadow-xl py-8 px-[80px] z-10 animate-fade-in-down">
          <div className="flex items-start gap-[120px] lg:gap-[160px] justify-start">
            {Object.entries(COLLECTION_DATA).map(([category, items]) => (
              <div key={category} className="flex flex-col gap-4">
                <h3 className="text-[#FE8204] text-[18px] font-bold uppercase">{category}</h3>
                <ul className="flex flex-col gap-2.5">
                  {items.map((item) => (
                    <li key={item}>
                      <Link 
                        to={`/collection/${item.toLowerCase().replace(/ & /g, '-').replace(/, /g, '-').replace(/ /g, '-')}`}
                        className="text-black text-[15px] font-bold uppercase hover:text-[#FE8204] transition-colors"
                        onClick={() => setActiveDropdown(null)}
                      >
                        {item}
                      </Link>
      ...
      {/* How it Works Mega Menu */}
      {activeDropdown === 'how-it-works' && (
        <div className="hidden md:block absolute top-full left-1/2 -translate-x-1/2 w-[95%] max-w-[1340px] bg-white border border-black rounded-[40px] shadow-xl py-6 px-[100px] z-10 animate-fade-in-down">
          <div className="flex justify-between items-center w-full">
            {HOW_IT_WORKS_DATA.map((item) => (
              <Link 
                key={item}
                to={`/${item.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-')}`}
                className="text-black text-[16px] font-bold uppercase hover:text-[#FE8204] transition-colors"
                onClick={() => setActiveDropdown(null)}
              >
                {item}
              </Link>
      ...
      {/* MOBILE MENU DRAWER (Unchanged, remains fully responsive) */}
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full bg-white border-t border-gray-200 px-4 py-6 flex flex-col gap-6 h-screen overflow-y-auto z-40">
          <Link to="/new-arrival" className="text-black text-[20px] font-bold uppercase" onClick={() => setIsMobileMenuOpen(false)}>NEW ARRIVAL</Link>
          ...
          <div className="flex flex-col gap-4">
            ...
            {mobileExpanded === 'collection' && (
              <div className="flex flex-col gap-6 pl-4 border-l-2 border-[#FE8204] ml-2">
                 {Object.entries(COLLECTION_DATA).map(([category, items]) => (
                    <div key={category} className="flex flex-col gap-3">
                      <h4 className="text-[#FE8204] text-[16px] font-bold uppercase">{category}</h4>
                      {items.map(item => (
                        <Link 
                          key={item} 
                          to={`/collection/${item.toLowerCase().replace(/ & /g, '-').replace(/, /g, '-').replace(/ /g, '-')}`}
                          className="text-gray-700 text-[14px] font-bold uppercase hover:text-[#FE8204]"
                          onClick={() => setIsMobileMenuOpen(false)}
                        >
                          {item}
                        </Link>
